      ...
      {phase === 2 && (
        <div className="card anim-fade-up mobile-card">
          ...
          <div className="mobile-section">
            ...
            <div className="field"><textarea className="input manual-textarea" placeholder={"\\settitle{1}{注意事項…}\n\\daimon{1} 小問集合 \\hai{25}\n\\toi{1} …\n…\n第1回 マークシート / 自己採点欄 / 解答・解説"} value={llmOutput} onChange={e => setLlmOutput(e.target.value)} />{llmOutput.trim() && <div className="latex-validation-hints"><span className={llmOutput.includes('\\settitle')?'hint-ok':'hint-warn'}>{llmOutput.includes('\\settitle')?'✅':'⚠️'} 表紙・注意事項</span><span className={llmOutput.includes('\\daimon')?'hint-ok':'hint-warn'}>{llmOutput.includes('\\daimon')?'✅':'⚠️'} 大問</span><span className={llmOutput.includes('マークシート')?'hint-ok':'hint-warn'}>{llmOutput.includes('マークシート')?'✅':'⚠️'} マークシート</span><span className={(llmOutput.includes('\\soldai')||llmOutput.includes('解答・解説')||llmOutput.includes('解答解説'))?'hint-ok':'hint-warn'}>{(llmOutput.includes('\\soldai')||llmOutput.includes('解答・解説')||llmOutput.includes('解答解説'))?'✅':'⚠️'} 解答・解説</span></div>}</div>
          </div>
          <div className="mobile-sticky-action">

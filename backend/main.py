...
=== 接触・リアリティの厳密ルール(厳守) ===
- 物体が面(床・斜面・レール・台)に「乗る」場合は、物体の底辺の y 座標を面の y 座標に厳密に一致させる(隙間も食い込みも禁止)。
  床は \\groundstrip、台車は \\cart、球は \\ballobj を使い、面の高さと物体の底を同じ値にする。
- 斜面上の物体は斜面に密着させる(scope の shift＋rotate=θ で斜面に合わせ、底辺を斜面線に重ねる)。浮かせない・めり込ませない。
- 壁・天井に接する物体やばねも、接点座標を完全に一致させる。ばね(coil spring)は壁の面と物体の面の間にちょうど渡す。

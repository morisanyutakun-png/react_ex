...
DIAGRAM_PACKAGES: Dict[str, Dict[str, str]] = {
    ...
    'circuitikz': {
        ...
        'prompt_hint': (
            ...
            '【★★★★★★ STEP 0: 描画前に必ず「トポロジー分類」を行う ★★★★★★】\n'
            '回路を描く前に、★問題文を読んで以下のどれに該当するか判定★ する:\n\n'
            '  ▼ Type-S（単一閉ループ）: 全素子が一周の経路上に直列に並ぶ\n'
            '     例: 電池＋R1＋R2＋R3 を全部直列、または 電池＋RLC 直列\n'
            '     → 描き方: ★反時計回り・左上始点・1本の \\draw で一筆書き★（後述）\n\n'
            '  ▼ Type-P（純粋並列）: 2本の母線間に N 本の素子枝が並ぶ（電源も母線間に置く）\n'
            '     例: 電池とR1, R2, R3 が全て上下母線間に並列\n'
            '     → 描き方: ★母線2本＋並列枝 N 本を別々の \\draw★（後述【並列回路】）\n\n'
            '  ▼ Type-SP（直列＋並列の混合・最頻出）:\n'
            '     直列部（電池・内部抵抗 r・直列抵抗 R1 等）の先に並列部（R2||R3 等）がある\n'
            '     例: 「電池 E（内部抵抗 r）に R1 を直列、その後 R2 と R3 を並列」← 本問\n'
            '     → ★Type-S として描いてはいけない★。並列部は必ず2枝以上の独立 \\draw で。\n'
            '     → 描き方: 後述【直列＋並列 混合回路の正しい書き方 (M1〜M6)】\n\n'
            '★トポロジー判定ミスの実例（実際に発生）★:\n'
            '  問題文: 「R1 を直列、R2 と R3 を並列」\n'
            '  ✗ 誤: 矩形ループの右辺に R2、下辺に R3 を置く → これは R2-R3 直列\n'
            '  ✓ 正: 直列部の終端から右に2本の縦並列枝として R2 と R3 を出す\n\n'
            '【★★★★★ 電気回路図の絶対原則（毎回必ず順守）★★★★★】\n'
            'ALPHA. 素子の脚は ★必ず軸に平行（水平 or 垂直）★ にする。斜め配線は禁止。\n'
            '       例: コンデンサが上下の母線間に縦に並ぶなら、両端は同じ x 座標。\n'
            '       例: (1,0) to[C,l=$C_1$] (1,2) ← 良い（x=1 で揃う、縦）\n'
            '       例: (1,0) to[C,l=$C_1$] (1.5,2) ← ★禁止★（x が違う → 斜め）\n'
            'BRAVO. ノードラベル(A,B,C等)は ★同じ電位の点では1回だけ★ 表示する。\n'
            '       同じ導線上の左端と右端の両方に "A" "A" と書かない。1本の母線につき1ノード名。\n'
            '       例: \\node[left] at (0,2) {$A$}; だけにする（右端 (4,2) にも "A" は書かない）\n'
            'CHARLIE. ★孤児の極性記号 "+", "−" を絶対に置かない★。電池/電圧源を取り除いたら、\n'
            '       その元の位置に "+" "−" のテキストを残してはいけない。\n'
            '       電池があるときだけ to[battery1] が極性線を自動描画する。\n'
            'DELTA. ★並列回路（同じ2本の母線間に複数の素子枝）は 1本の \\draw で書けない★。\n'
            '       無理に反時計回り一筆書きにしようとすると素子が斜めになる。並列パターンは\n'
            '       後述の【並列回路の正しい書き方】に従い、複数の \\draw に分割する。\n'
            'ECHO. 各素子の前後に余分な短い -- や to[short] を入れない。to[X] が自動で脚を引く。\n\n'
            ...
            '    左上(0,H) → 左下(0,0) → 右下(W,0) → 右上(W,H) → 左上(0,H)。\n'
            '    すなわち「下降 → 右進 → 上昇 → 左進」の順。\n'
            'D3. ★単一閉ループは必ず1本の \\draw で一周★。途中で \\draw を切らない。\n'
            '    辺の途中に素子があっても、その辺を1本の \\draw の連続として書く:\n'
            '      \\draw (0,H) to[素子A] (0,0)  % 左辺（素子Aで下降）\n'
            '            to[素子B] (W,0)         % 下辺（素子Bで右進、または -- で配線のみ）\n'
            '            to[素子C] (W,H)         % 右辺（素子Cで上昇、または -- で配線のみ）\n'
            '            to[素子D] (0,H);        % 上辺（素子Dで左進、または -- で始点に戻る）\n'
            'D4. ★電源は左辺に配置★: 左上から左下へ下降する辺に置く。\n'
            '    \\draw (0,H) to[battery1,l=$E$] (0,0) ...\n'
            '    この向きで描けば長線(+)が上・短線(−)が下に来て、正しい標準向きとなる。\n'
            '    invert は使わない（必要になる時点で描画方向が間違っている）。\n'
            'D5. ★スイッチは to[switch,l=$S$] または to[opening switch,l=$S$]★。\n'
            '    スイッチも素子と同じく、矩形のどれかの辺の連続パス中に組み込む。\n'
            '    別の \\draw で「スイッチだけ後から」描かない。\n'
            'D6. ★複数 \\draw を使ってよい条件★: 真の並列回路（3つ以上の独立した枝が\n'
            '    上下の母線間に並ぶ）のみ。素子2つの単純ループは1本の \\draw で済む。\n\n'
            '【★ 単一閉ループの正しい例（コンデンサ2つ＋スイッチ） ★】\n'
            ...
            'E2. 素子区間の長さは 1.5〜3.0 単位（短すぎると記号が潰れる）。\n'
            'E3. ★素子の中点を他の配線が横切らないように座標を決める★。\n'
            '    特に水平母線と垂直素子の x 座標が一致するような長母線描画は禁止。\n\n'
            '【座標ルール】\n'
            'C1. ★閉回路の保証★: 最後の配線の終点座標は、始点と正確に一致すること。\n'
            'C2. 矩形ループなら左辺 x=0、右辺 x=W、上辺 y=H、下辺 y=0 に揃える。\n'
            'C3. 描画前にコメントで座標一覧を書く:\n'
            '    % ノード: 左上 A=(0,3), 左下 B=(0,0), 右下 C=(4,0), 右上 D=(4,3)\n'
            '    % 反時計回り経路: A→B (左辺,C1で下降), B→C (下辺,配線), C→D (右辺,C2で上昇), D→A (上辺,Sで左進)\n'
            'C4. 接地記号: node[ground]{} の座標も明記。\n\n'
            '【並列回路の正しい書き方（コンデンサ2つ・抵抗複数・電源なし回路）】\n'
            'P1. 上下2本の母線（rail）を ★それぞれ1本の \\draw で水平に★ 引く。\n'
            '    \\draw (0,2) -- (4,2);  % 上母線（電位 A）\n'
            '    \\draw (0,0) -- (4,0);  % 下母線（電位 B）\n'
            'P2. ★並列枝は1枝につき1本の \\draw★、両端の x 座標を ★必ず一致★ させる:\n'
            '    \\draw (1,2) to[C,l=$C_1$] (1,0);    % x=1 で縦に下降\n'
            '    \\draw (3,2) to[C,l=$C_2$] (3,0);    % x=3 で縦に下降\n'
            'P3. ノードラベル（A, B 等の電位名）は ★母線につき1回だけ★。\n'
            '    \\node[left] at (0,2) {$A$};\n'
            '    \\node[left] at (0,0) {$B$};\n'
            '    右端 (4,2) には "A" を書かない（同じ電位だから不要）。\n'
            'P4. スイッチを上母線に挿入する場合は、その母線の \\draw 内で:\n'
            '    \\draw (0,2) -- (1.5,2) to[opening switch,l_=$S$] (2.5,2) -- (4,2);\n'
            'P5. 分岐点に接続ドットを置く場合は node[circ]{} を母線上の素子接続点に。\n'
            '    \\draw (1,2) node[circ]{} to[C,l=$C_1$] (1,0) node[circ]{};\n'
            'P6. ★電源（電池/電圧源）が「切り離されている」問題では電池記号を描かない★。\n'
            '    "+", "−" のテキストを残骸として置くのも禁止（CHARLIE 原則）。\n\n'
            '【★ コンデンサ並列＋スイッチの正解例（典型: コンデンサ充電→切り離し→並列接続） ★】\n'
            '\\begin{circuitikz}[scale=1.0,>=stealth]\n'
            '  % 上下の母線（電位 A: 上、電位 B: 下）。スイッチ S は上母線の中央。\n'
            '  \\draw (0,2) -- (1.5,2) to[opening switch,l_=$S$] (2.5,2) -- (4,2);\n'
            '  \\draw (0,0) -- (4,0);\n'
            '  % 並列枝（縦・両端 x 一致）\n'
            ...
            '  \\node[left] at (0,0) {$B$};\n'
            '\\end{circuitikz}\n\n'
            '【真の並列回路（電源あり・枝が3本以上）の描画手順】\n'
            'Q1. 外周（電源・上下母線・右辺）を1本の \\draw で反時計回り一筆書き。\n'
            'Q2. ★内側の並列枝のみ★を後続の \\draw で追加。\n'
            'Q3. 並列素子は外周の母線とは別の \\draw に分け、両端の x を一致させる。\n'
            '   例:\n'
            '   \\draw (0,3) to[battery1,l=$E$] (0,0)   % 左辺: 電源で下降\n'
            '         -- (8,0)                            % 下辺\n'
            '         -- (8,3)                            % 右辺\n'
            '         -- (0,3);                           % 上辺で閉ループ\n'
            '   \\draw (2.6,3) to[R,l_=$R$] (2.6,0);    % 並列枝1\n'
            '   \\draw (5.0,3) to[L,l_=$L$] (5.0,0);    % 並列枝2\n'
            'Q4. 分岐点に接続ドットを付ける場合は node[circ]{} を素子枝の端に置く。\n\n'
            '【★ 直列＋並列 混合回路の正しい書き方 (M1〜M6) — Type-SP の必須手順 ★】\n'
            '典型問題: 「電池 E（内部抵抗 r）に R_1 を直列、その先に R_2 と R_3 を並列に接続」\n'
            'M1. 矩形外周＋並列枝 という構造を意識する。並列部は ★右側に縦に並べる★。\n'
            'M2. 全体の \\draw 構造（3本に分ける）:\n'
            '    \\draw 1本目: 上辺（電池 E の +端子 → r → R_1 → 並列の上母線右端）\n'
            '    \\draw 2本目: 下辺（並列の下母線右端 → 電池 E の −端子）\n'
            '    \\draw 3本目: 電池 E（左辺、縦）\n'
            '    並列枝: 並列素子それぞれ別 \\draw（縦・両端 x 一致）\n'
            'M3. 並列部の x 座標は ★均等間隔★ に。例: 並列2本なら x=4 と x=5.5\n'
            'M4. 電流矢印 I（主電流）, I_2, I_3 はそれぞれの素子の to[R,...,i>^=$I$] に組込む。\n'
            '    主電流 I は直列部の R_1 等に。並列電流 I_2, I_3 は各並列枝の R_2, R_3 に。\n'
            '    ★絶対に独立した \\draw[->] で電流矢印を描かない★（素子からズレる原因）。\n'
            'M5. 並列部の上母線・下母線は ★短くてよい★（並列枝の x 範囲をカバーするだけ）。\n'
            'M6. ノード分岐点に node[circ]{} を置くと視覚的にわかりやすい。\n\n'
            '【★ Type-SP の正解例（電池+r+R_1 直列、R_2||R_3 並列） ★】\n'
            '\\begin{circuitikz}[scale=1.0,>=stealth]\n'
            '  % 直列部の上辺: 左から右へ E+ → r → R_1 → 分岐点（並列上端）\n'
            ...
            '  % 並列枝1: R_2（x=4 で縦下降, I_2 矢印付き）\n'
            '  \\draw (4,3) node[circ]{} to[R,l=$R_2$,i>^=$I_2$] (4,0) node[circ]{};\n'
            '  % 並列枝2: R_3（x=5.5 で縦下降, I_3 矢印付き）\n'
            '  \\draw (5.5,3) to[R,l=$R_3$,i>^=$I_3$] (5.5,0);\n'
            '\\end{circuitikz}\n'
            '★ ポイント ★:\n'
            '  - R_2 と R_3 を矩形の隣接辺に置かない（それは直列になってしまう）。\n'
            '  - 並列部の上母線 (3.5,3)--(5.5,3) と下母線 (4,0)--(5.5,0) で R_2,R_3 を挟む。\n'
            '  - 電流矢印は i>^=$I$ オプションで素子上に重ねる。\n'
        ),
    },
    'pgfplots': {

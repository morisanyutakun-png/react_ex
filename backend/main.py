...
F10. 力の分解: 元ベクトル=実線, 分解成分=dashed, 直角マーク=小正方形(0.2単位)
    \draw[dashed,>=stealth,->] (P)--(Px) node[midway,below]{$F_x$};
    \draw ($(Px)+(0,0.2)$)--++(0.2,0)--++(0,-0.2); % 直角マーク
...
F13. 力ベクトルのラベルは必ず node[midway,right] または node[midway,left] で矢印の中点に配置する。
...
\begin{tikzpicture}[>=stealth,scale=1.2]
  ...
  \begin{scope}[shift={(1.73,1.00)},rotate=30]
    ...
    \draw[->,red,very thick] (0,0.6)--++(0,1.0) node[midway,right]{$N$};
    % 摩擦力: scope内で斜面に沿って上向き
    \draw[->,green!60!black,thick] (-0.3,0.3)--++(-1.0,0) node[midway,left]{$f$};
  \end{scope}
  % 重力: 世界座標で真下（物体中心の世界座標から）
  \draw[->,blue,very thick] (1.58,1.26)--++(0,-1.0) node[midway,right]{$mg$};
\end{tikzpicture}
...
\begin{tikzpicture}[>=stealth,scale=1.2]
  ...
  \begin{scope}[shift={(1.06,1.06)},rotate=45]
    ...
    \node at (0,0.25) {$m$};
    \draw[->,red,very thick] (0,0.5)--++(0,0.9) node[midway,right]{$N$};
  \end{scope}
  \draw[->,blue,very thick] (0.88,1.24)--++(0,-0.9) node[midway,right]{$mg$};
  % 運動方向（斜面下向き）
  \draw[->,orange,thick,dashed] (1.24,0.88)--++(0.35,-0.35) node[midway,right]{$a$};
\end{tikzpicture}
...
\begin{tikzpicture}[>=stealth,scale=1.0]
  ...
  \draw[->,blue,thick] (4.33,0.3)--++(0,-0.6) node[midway,right]{$m_B g$};
\end{tikzpicture}
...
\begin{tikzpicture}[>=stealth,scale=1.1]
  ...
  \node at (2.0,0.5) {$m$};
  \draw[->,red,very thick] (2.5,0.5)--++(1.2,0) node[midway,above]{$F$};
  \draw[->,blue,thick] (2.0,0.5)--++(0,-0.8) node[midway,right]{$mg$};
  \draw[->,green!60!black,thick] (2.0,1.0)--++(0,0.7) node[midway,right]{$N$};
\end{tikzpicture}
...
\begin{tikzpicture}[>=stealth,scale=1.1]
  ...
  \node at (2.55,0.35) {$m$};
  \draw[->,red,thick] (2.9,0.35)--++(0.9,0) node[midway,above]{$F$};
  \node at (1.17,0.62) {$k$};
\end{tikzpicture}
...
\begin{tikzpicture}[>=stealth,scale=1.1]
  ...
  \node at (0,1.45) {$m$};
  \draw[->,blue,thick] (0,1.25)--++(0,-0.85) node[midway,right]{$mg$};
  \draw[->,red,thick] (0,1.65)--++(0,0.7) node[midway,right]{$T$};
...
\begin{tikzpicture}[>=stealth,scale=1.0]
  ...
  \node at (4.5,-0.95) {$m_B$};
  % 力の矢印
  \draw[->,blue,thick] (4.5,-1.3)--++(0,-0.6) node[midway,right]{$m_B g$};
  \draw[->,red,thick] (2.2,0.4)--++(0.8,0) node[midway,above]{$T$};
  \draw[->,blue,thick] (1.7,0)--++(0,-0.6) node[midway,right]{$m_A g$};
\end{tikzpicture}
...
\begin{tikzpicture}[>=stealth,scale=1.0]
  ...
  \node at (2.8,0.35) {$m_2$};
  % 力の矢印
  \draw[->,blue,thick] (2.2,0.5)--++(0,-0.6) node[midway,right]{$m_1 g$};
  \draw[->,blue,thick] (2.8,0.0)--++(0,-0.6) node[midway,right]{$m_2 g$};
\end{tikzpicture}
...
def _preprocess_tikz_code(tikz_code: str) -> str:
    ...
    # 8. 力ベクトルのラベルをmidway配置に変換（端点配置はモバイル縮小で崩れる）
    #    --++(x,y) node[below/above/right]{...} → node[midway,right]{...}
    #    --++(相対座標)パスのみ対象。軸ラベル等の--(絶対座標)は影響しない
    code = _re.sub(
        r'(--\+\+\([^)]*\))\s*node\[(below|above|right)\]',
        r'\1 node[midway,right]',
        code
    )
    code = _re.sub(
        r'(--\+\+\([^)]*\))\s*node\[left\]',
        r'\1 node[midway,left]',
        code
    )

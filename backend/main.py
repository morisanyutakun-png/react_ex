...
def _build_mock_exam_prompt(req: 'MockExamPromptRequest') -> str:
    ...
    spec = f"""あなたは大学入学共通テスト形式の模擬試験を作る、プロの問題作成者です。「{subject} 予想模試(1回分)」を、後述の preamble を一切改変せず用い、\\documentclass から \\end{{document}} までの完全な1つの LaTeX 文書として作成する(本文だけを埋める)。

=== 出題仕様 ===
- タイトル「第{round_no}回 予想問題」。大問 I〜IV の4題・各25点・計100点・60分。
...
3.マークシート: \\section*{{第{round_no}回 マークシート}}。★単列(2列の表)で、全番号 1〜{num_answers} を \\msrowT で1行ずつ(2段組の \\msrowTT はB5幅超で禁止):
    ...
    \\hline
    解答番号 & マーク欄 \\\\ \\hline
    \\msrowT{{1}}\\msrowT{{2}} … \\msrowT{{{num_answers}}}
    \\end{{tabular}}\\end{{center}}
4.自己採点欄: 番号・配点・自分の解答・得点の表＋合計欄(/100)。
5.解答・解説: \\section*{{第{round_no}回 解答・解説}} → まず配点表(正解一覧)→ \\soldai{{n}}{{大問名}} ごとに各問の正解 \\cn{{n}}・配点・簡潔な解説。配点合計はちょうど100。冒頭にも本書が非公式の予想問題である旨を記す。

=== 分量(必達) ===
問題本文(表紙の次〜第IV問末。マークシート・採点欄・解説を除く)だけで約28〜32ページ。各大問に背景説明(2〜4段落)＋会話文＋図2〜3個＋データ表を入れ、中身で厚くする(空白で水増ししない)。文書全体で図は最低8個(目安8〜12)、各大問に装置図/概念図を最低1つ。「(以下同様)」「(略)」等のプレースホルダ禁止。全 {num_answers} 問を実際に作問し、解説も全問分書く。
...
=== 接触・配置(厳守) ===
床・面に物体を「乗せる」時は高さを手計算せず必ず専用マクロを使う(接触が自動保証): \\groundstrip{{x1}}{{x2}}{{床y}}(床)・\\ballon{{x}}{{床y}}{{r}}(球)・\\boxon{{x}}{{床y}}{{幅}}{{高}}{{ラベル}}(箱)・\\block{{x}}{{床y}}(立体)・\\cart{{x}}{{床y}}{{ラベル}}(台車)。★生の \\ballobj を床に置いて中心yを手計算するの禁止(浮き/めり込み事故)。斜面上は \\begin{{scope}}[shift={{(接点x,接点y)}},rotate=θ] 内で底辺 y=0 の箱。壁・天井・ばねは接点座標を一致させる(coil spring は壁面と物体面の間にちょうど渡す)。物体・球・台車は必ず面に接して描く(隙間も食い込みも禁止)。
...
接触・向き・ロープの掛かり・ラベル位置が検証済みで、座標がずれても破綻しない。呼んだら上に同じ装置の線やラベルを重ねない(必要要素は内蔵済み)。
  ...
  ・屈折/全反射: \\refraction{{上の媒質}}{{下の媒質}}(界面・法線・入射/屈折光に加え入射角 i・屈折角 r を必ず弧で示す。i,r を点や文字だけにしない)
  ・放射性崩壊の直前: \\nucleusrest(静止核のみ。崩壊後の粒子/速度/運動量の矢印を描かない)
...
単独部品が要る時のみ \\pulley/\\cart/\\weightbox 等を使う。上記にない場面を自前で描く時も、面のy座標と物体の底のy座標を一致させ、ラベルを線・矢印・格子記号に重ねない。
...
1つの巨大な tikzpicture に詰め込まない(ラベルがずれる・ページ途中で割れる)。各選択肢を独立した小 tikz にし、tabular のセルに「\\cn{{n}}\\,(図)」で並べ、1つの table/center にまとめ各 \\cn を図の直前に置く(図とラベルを別ページに分けない)。グラフは \\gframe、記録テープの打点は \\dotrow を使う。
  ...
  打点例: \\cn{{1}}\\,\\dotrow{{0.3,1.6,2.9,4.2,5.5,6.6}}(等間隔=等速、右へ広がる=加速)。「ア・イ」組合せ表の例:
    \\begin{{center}}\\small\\renewcommand{{\\arraystretch}}{{1.4}}\\begin{{tabular}}{{|c|c|c|}}
    \\hline
    & ア & イ \\\\ \\hline

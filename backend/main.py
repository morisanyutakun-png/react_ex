            ...
            if isinstance(prompt, str) and ('\\documentclass' not in prompt):
                ...
                if _is_stem_subject(_subj_for_rules, orig_prompt_body):
                    ...
                    if _diagram_realism and extra_pkgs:
                        ...
                        elif _subj_for_rules in ('電気回路',) or '回路' in (_subj_for_rules + orig_prompt_body[:300]):
                            latex_instr += (
                                '\n【電気回路図の精度ルール】\n'
...
_STEM_KEYWORDS = frozenset({
    '物理', '数学', '電気回路', '応用情報', '情報',
    '微分', '積分', '線形代数', '微分方程式', 'フーリエ', 'ラプラス',
    'ベクトル', '行列', '関数', '方程式', '確率', '統計', '三角関数',
    '電気', '回路', '交流', '直流', '過渡', 'オペアンプ', 'ダイオード', 'トランジスタ',
    '論理回路', '半導体', 'キルヒホッフ', 'テブナン', 'ノートン',
    '力学', 'エネルギー', '波動', '原子', '電磁気', '熱力学',
    'アルゴリズム', 'データ構造', 'データベース', 'ネットワーク', 'セキュリティ',
    'math', 'physics', 'electrical', 'circuit', 'engineering', 'informatics',
})
# 非対応科目（明示拒否）
...
# 工学系特化: 物理・数学・電気回路は図表・数式が多いので standard、応用情報は lite
_SUBJECT_TIER_MAP = {
    '応用情報': 'lite', '情報': 'lite',
    '物理': 'standard', '数学': 'standard', '電気回路': 'standard',
}
...
"""
    ...
    return f"""あなたは工学系教育（物理・数学・電気回路・応用情報）の問題作成エキスパートです。
大学工学部の演習問題・院試・応用情報技術者試験・電験など、工学系の実問題と同等の品質で
練習問題を作成してください。図表・回路図・数式が必要な場合は積極的に活用してください。

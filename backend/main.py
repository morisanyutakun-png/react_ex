                    ...
                    if _diagram_realism and extra_pkgs:
                        if _is_physics_subject(_subj_for_rules, orig_prompt_body):
                            latex_instr += (
                                ...
                                '- 力ベクトル: -{Stealth[length=4mm,width=3mm]} で統一\n'
                                '- 床面: pattern=north east lines でハッチング\n'
                                '- 線の太さ: 主要=thick, 補助=thin, 寸法=very thin\n'
                            )
                        elif _subj_for_rules in ('電気回路', '電子回路', '電磁気学',
                                                 '制御工学', '信号処理', '通信工学',
                                                 'ディジタル回路') or any(
                                kw in (_subj_for_rules + orig_prompt_body[:300])
                                for kw in ('回路', 'オペアンプ', '伝達関数', '変調')):
                            latex_instr += (
                                '\n【電気回路図の精度ルール】\n'
                                '- 回路図は CircuiTikZ を使う（\\begin{circuitikz}...\\end{circuitikz}）\n'
                                '- 反時計回り・左上始点・単一閉ループは1本の \\draw で一筆書き\n'
                                '- 素子は (始点) to[素子] (終点) の連続。前後に余計な -- を入れない\n'
                                '- 電源は左辺に: \\draw (0,H) to[battery1,l=$E$] (0,0) で +上 −下\n'
                                '- 並列枝が3本以上のときのみ \\draw を分割（母線→素子枝の順）\n'
                                '- ノードラベル: to[R,l=$R$] / to[C,l_=$C$] で素子名を明示\n'
                            )
                        else:
                            # 数学等の一般STEM
                            latex_instr += (
...
# 工学系特化: 電気電子・情報・機械の横断12科目
_STEM_SUBJECTS = frozenset({
    '物理', '数学',
    '電気回路', '電子回路', '電磁気学',
    '制御工学', '信号処理', '通信工学',
    'ディジタル回路', 'アルゴリズム', 'プログラミング', '応用情報',
    '情報',  # 互換のため残置（応用情報の旧称データに対応）
    '数学IA', '数学IIB', '数学III', '数学I', '数学II', '数学A', '数学B', '数学C',
})
_STEM_KEYWORDS = frozenset({
    # 教科名
    '物理', '数学', '電気回路', '電子回路', '電磁気', '制御', '信号処理',
    '通信', 'ディジタル', '論理回路', 'アルゴリズム', 'プログラミング', '応用情報', '情報',
    # 数学キーワード
    '微分', '積分', '線形代数', '微分方程式', 'フーリエ', 'ラプラス', 'z変換',
    'ベクトル', '行列', '関数', '方程式', '確率', '統計', '三角関数',
    # 物理キーワード
    '力学', 'エネルギー', '波動', '原子', '熱力学', '流体', '振動',
    # 電気電子キーワード
    '電気', '回路', '交流', '直流', '過渡', '共振', 'インピーダンス',
    'オペアンプ', 'ダイオード', 'トランジスタ', 'MOSFET', 'FET',
    '半導体', 'キルヒホッフ', 'テブナン', 'ノートン',
    'PID', '伝達関数', 'ボード', 'ナイキスト',
    'FFT', 'DFT', 'FIR', 'IIR', '変調', 'PSK', 'QAM',
    # 情報キーワード
    'データ構造', 'データベース', 'ネットワーク', 'セキュリティ',
    'スタック', 'キュー', '木構造', 'ハッシュ', '計算量',
    # 英語
    'math', 'physics', 'electrical', 'electronic', 'circuit', 'control',
    'signal', 'communication', 'digital', 'algorithm', 'programming',
    'engineering', 'informatics',
})
# 非対応科目（明示拒否）
_NON_STEM_SUBJECTS = frozenset({
    '英語', '国語', '社会', '化学', '生物', '地学', '理科',
    '日本史', '世界史', '地理', '公民', '倫理', '政経',
    '現代文', '古文', '漢文', '小論文',
    'english', 'japanese', 'history', 'geography', 'chemistry', 'biology',
})
...
_LATEX_PHYSICS_DIAGRAM_RULES = (
    ...
    "PD2. ★★ 力ベクトル・矢印の厳密ルール:\n"
    "     矢印スタイル: \\draw[-{Stealth[length=4mm,width=3mm]},thick] で作用点から描く。\n"
    "     ×誤: \\draw[->] ←矢先が小さすぎて視認不能。\n"
...
def _build_stem_system_prompt(subject: str, prompt_text: str,
    ...
    if is_physics:
        parts.append(
            ...
            '- ★★ 力ベクトル・矢印の厳密ルール:\n'
            '  1. 矢印スタイル: \\draw[-{Stealth[length=4mm,width=3mm]},thick,<色>]\n'
            '     ×誤: \\draw[->]  ←矢先が小さすぎて見えない\n'
            ...
            '  5. ラベル位置: 矢印とは別の \\node コマンドで中点に配置（node[midway]はパスに載せない）\n'
            '  6. 力の分解: 元ベクトル=実線, 分解成分=dashed, 直角マーク=小正方形(0.2単位)\n'
            '     \\draw[dashed,-{Stealth[length=4mm,width=3mm]}] (P) -- (Px) node[below]{$F_x$};\n'
    ...
    if is_physics:
        checklist += (
            ...
            '☐ 力ベクトルの矢印: -{Stealth[length=4mm,width=3mm]}を使っているか（->は禁止）\n'
...
DIAGRAM_PACKAGES: Dict[str, Dict[str, str]] = {
    'tikz': {
        ...
        'prompt_hint': (
            ...
            '8. 矢印: -{Stealth[length=4mm,width=3mm]} で統一。力・速度などのベクトル量は太い矢印 [very thick] を使う。\n'
    ...
    'circuitikz': {
        ...
        'prompt_hint': (
            ...
            '  to[C,l=$C$]            % コンデンサ\n'
            '  to[L,l=$L$]            % インダクタ（コイル）\n'
            '  to[battery1,l=$E$]     % 直流電池（長線=+、短線=−）\n'
            '  to[sV,l_=$E$]          % 交流電圧源（円に sin 波）\n'
            '  to[V,l=$V$]            % 一般電圧源\n'
            '  to[D]                  % ダイオード\n'
            '  to[switch,l=$S$]       % 閉スイッチ\n'
            '  to[opening switch,l=$S$] % 開スイッチ\n'
            'オプション:\n'
            '  l=$X$        素子の上または横に X を表示\n'
            '  l_=$X$       素子の下または逆側に X を表示\n'
            '  v=$V_1$      電圧矢印を素子に重ねて表示\n'
            '  i>^=$I$      電流矢印を素子の流れ方向に表示\n'
            '  *-*          両端に接続ドット\n'
            '\n'
            ...
            '絶対原則（毎回必ず順守）\n'
            '======================================================================\n'
            'A1. ★全配線は軸に平行★（水平 or 垂直のみ）。斜め配線は完全禁止。\n'
            '    例: (0,1.5) to[L] (3,3) ← x も y も違う → 斜め（禁止）\n'
            '    異なる x かつ異なる y の点を結ぶときは、L字経路を使う:\n'
            '      \\draw (0,1.5) -- (0,3) -- (3,3);  % 縦→横で接続\n'
            '\n'
            'A2. ★素子の両端は同じ軸上★ にする。\n'
            '    縦素子: 両端の x を同一に。 (1,0) to[C] (1,2) ← OK\n'
            '    横素子: 両端の y を同一に。 (0,3) to[R] (4,3) ← OK\n'
            '\n'
            'A3. ★1素子=1区間★。素子の前後に余計な -- や to[short] を入れない。\n'
            '    to[R] が自動で素子の脚を引くので、追加の配線は不要。\n'
            '\n'
            'A4. ★同じ電位の点には1回だけラベル★（A, B など）。\n'
            '    1本の母線につき1ノード名。両端に "A" "A" と二重表示しない。\n'
            '\n'
            'A5. ★孤児の "+", "−" 記号を絶対に置かない★。\n'
            '    電池を取り除いたら、極性記号テキストも全部消す。\n'
            '\n'
            'A5b. ★コンデンサの極性は to[C,v=$V$] オプションで自動描画★。手動禁止。\n'
            '     コンデンサに電圧の向き（極板の +, −）を示したい場合は:\n'
            '       \\draw (0,2) to[C,l=$C_1$,v=$V_1$] (0,0);  % v=$V_1$ で極性矢印が自動表示\n'
            '       \\draw (0,2) to[C,l=$C_1$,v>=$V_1$] (0,0); % >= で上向き矢印\n'
            '     ★ \\node {$+$}; \\node {$-$}; を素子の内部や横に手動で置くのは絶対禁止★\n'
            '     極性のあるコンデンサ（電解コンデンサ）を描きたい場合は to[eC,l=$C$] を使う。\n'
            '\n'
            'A6. ★電圧 V_1, V_2 等は to[R,...,v=$V$] オプションで描く★。\n'
            '    独立した \\draw[->] で電圧矢印を後描きしない（素子上を斜めに横切るバグの元）。\n'
            '\n'
            'A7. ★電流 I, I_2 等は to[R,...,i>^=$I$] オプションで描く★。\n'
            '    独立した \\draw[->] で電流矢印を後描きしない。\n'
            '\n'
            'A8. ★素子は CircuiTikZ プリミティブのみで描く★。手描き禁止。\n'
            '    ★絶対に \\draw (a,b) rectangle (c,d) で抵抗・コンデンサ・電源を描かない★\n'
            '    抵抗は to[R]、コンデンサは to[C]、電池は to[battery1]、コイルは to[L] が\n'
            '    自動的に正しい記号（抵抗=長方形、コンデンサ=平行2本線、電池=長短2本線）を描画する。\n'
            '    ★手動で矩形を描き「+」「−」のテキストを内部や外側に置くのは絶対禁止★\n'
            '    ★ノードラベル C_1, C_2 等を素子の外に手動で置くのも禁止★。\n'
            '    to[C,l=$C_1$] のオプション l= で記号横に自動配置される。\n'
            '\n'
            'A9. ★ノードラベル位置のオプションを活用★。\n'
            '    l=$X$    素子の上または横にラベル X（標準）\n'
            '    l_=$X$   素子の下または反対側にラベル X\n'
            '    l^=$X$   素子の上にラベル X（明示的）\n'
            ...
            '======================================================================\n'
            'C1. \\begin{circuitikz} と \\end{circuitikz} が対応している（数を数える）。\n'
            'C2. すべての \\draw 文末に ; がある。\n'
            'C3. 添字付き素子名は math モード内に書く: l=$R_1$ ← OK / l=R_1 ← NG\n'
            '    2文字以上の添字は {} で囲む: l=$R_{12}$ ← OK / l=$R_12$ ← NG\n'
            'C4. 半角の ( ) , 0-9 を使う。全角 （ ） ０ などは禁止。\n'
            'C5. node[circ]{} の {} は ★空でも必須★。node[circ] 単独はNG。\n'
            'C6. ノードラベル中の特殊文字 (%, &, #, _) はそのまま使わない。\n'
            '    %  → \\%   &  → \\&   _ は数式内で _{...} として使用\n'
            'C7. \\usepackage, \\usetikzlibrary は書かない（preamble で読み込み済み）。\n'
            'C8. 閉ループ確認: 各 \\draw の終点座標を全部書き出し、\n'
            '    閉じた多角形を形成するか目視確認する。\n'
            '\n'
            '======================================================================\n'
            '【陥りやすいバグの実例 — 絶対にやらない】\n'
            '======================================================================\n'
            '✗ 電池を矩形ループの内部や中央に置き、配線を斜めに引いて他の素子につなぐ\n'
            '   → 電池は ★必ず左辺 (0,0)-(0,H) の縦★ に置く。Type-S/SP の場合。\n'
            '✗ Type-SP で R_2 を矩形の右辺、R_3 を下辺に置く → R_2-R_3 直列化\n'
            '   → R_2 と R_3 は ★並列枝として縦に2本★ 描く。\n'
            '✗ \\draw を辺ごと 4本に分け、辺の途中に素子を後から差し込む\n'
            '   → 母線と素子の脚が二重描画され、貫通バグ発生。\n'
            '✗ V_1, V_2 ラベルのために独立した \\draw[->] (x1,y1)--(x2,y2) を引く\n'
            '   → 素子上を斜めに横切る線が出る。 v=$V_1$ オプションで組込む。\n'
            '✗ to[battery1] の後に to[short] や -- を不要に追加\n'
            '   → 電池の脚が二重になり斜めに見える。\n'
            '✗ 電池記号を消したのに極性 "+", "−" のテキストを残す（孤児）\n'
            '✗ \\draw (a,b) rectangle (c,d) でコンデンサ・抵抗・電池を「描いたつもり」になる\n'
            '   → 本物の to[C], to[R], to[battery1] を使う。矩形描画は素子ではない。\n'
            '✗ コンデンサの極性を \\node {$+$}, \\node {$-$} のテキストで配置する\n'
            '   → to[C,v=$V$] オプションが極性矢印を自動描画する。\n'
            '✗ 素子ラベル C_1, R_2 等を \\node at (x,y) {...} で手動配置する\n'
            '   → to[C,l=$C_1$] の l= オプションが自動配置する。\n'
        ),
    },
    'pgfplots': {
...
\newcommand{\groundstrip}[3]{%
  ...
  \draw[rail] (#1,#3)--(#2,#3);}
% ── 面に「乗せる」物体(接触を自動保証・高さ計算不要。\ballobj を床に直接置かない) ──
% 球: \ballon{中心x}{面y}{半径} → 中心を面y+半径に置くので底が必ず面に接する
\newcommand{\ballon}[3]{\shade[ball color=black!18] (#1,{#2+#3}) circle (#3);\draw[edgethin] (#1,{#2+#3}) circle (#3);}
% 箱(底辺=面y・立体風): \boxon{中心x}{面y}{幅}{高さ}{ラベル}
\newcommand{\boxon}[5]{\shadedraw[edge,top color=white,bottom color=black!20] ({#1-#3/2},#2) rectangle ({#1+#3/2},{#2+#4});\node[font=\footnotesize] at (#1,{#2+#4/2}){#5};}
...
% 記録テープの打点パターン(選択肢用・固定サイズ)。#1=ドットのx座標リスト(カンマ区切り)
...
def _build_mock_exam_prompt(req: 'MockExamPromptRequest') -> str:
    ...
    num_answers = max(24, min(36, int(req.num_answers or 30)))
...
- 第2問〜第4問: 探究活動・実験考察・文章読解型を中心に、誘導に沿って解く構成。
  会話文(太郎・花子など)・実験の目的や装置の説明・測定データの表・グラフ選択・組合せ選択(ア・イを表から選ぶ)を必ず織り込む。

★★★ 分量(最重要・必達): 問題冊子(表紙の次〜第IV問の最後)だけで「約28〜32ページ」になる本格的な分量にする。
  これはマークシート・自己採点欄・解答解説を含まない、問題本文だけのページ数である。
  ページ数を満たすため、各大問を次のように厚くする(空白で水増しせず、中身で増やす):
    ・第2〜4問は、実験の背景・目的・手順・装置の説明文(各2〜4段落)＋会話文を必ず付ける。
    ・各大問に図を2〜3個(装置図・グラフ・概念図)入れる。文書全体で図は最低12個。
    ・測定データの表・選択肢の表を各大問に入れる。
    ・1つの設問文は短く切らず、状況設定・条件を丁寧に書く。
- 解答番号は文書全体で通し番号。合計 {num_answers} 個(最低28個)とすること。各空欄は \\mb{{n}} で表示。
- 設問数(本番準拠・必ずこの規模を満たす。間引かない):
    第1問 … 問1〜問6(解答番号 1〜6)
    第2問 … 問1〜問7(解答番号 7〜13 程度)
    第3問 … 問1〜問8(解答番号 14〜21 程度。A・Bの二部構成にしてよい)
    第4問 … 問1〜問7(解答番号 22〜{num_answers}。A・Bの二部構成にしてよい)

★ 考察(思考力)問題を必ず入れる: 公式に数値を代入するだけの問題ばかりにしない。
  第2〜4問にはそれぞれ2問以上、次のような「ひねった・考えさせる」設問を含める:
    ・なぜその実験方法/装置/近似を使うのか(理由・妥当性)を問う。
    ・グラフや測定データを読み取り、傾き・切片・面積の物理的意味を問う。
    ・条件を変えたら結果がどう変わるか(定性的予測)、誤差の原因や測定の工夫を問う。
    ・2つの場面・素子・物体を比較させる、複数ステップの推論を要する問題。
  選択肢には「もっともらしい誤答(よくある勘違い)」を必ず混ぜ、単純消去法で解けないようにする。
- 難易度方針: {difficulty}
{theme_block}
...
=== 接触・リアリティの厳密ルール(最重要・厳守) ===
★物体を床・面に「乗せる」ときは、中心や底の高さを自分で計算しない。必ず次の専用マクロを使う(接触が自動保証される)。
    \\groundstrip{{x1}}{{x2}}{{床y}}      … 床(ハッチング＋基準線)
    \\ballon{{中心x}}{{床y}}{{半径}}      … 球(中心が床y+半径に置かれ、底が必ず床に接する)
    \\boxon{{中心x}}{{床y}}{{幅}}{{高さ}}{{ラベル}} … 箱(底辺が床yに一致)
    \\block{{x}}{{床y}}                  … 立体ブロック(底辺が床y)
    \\cart{{x}}{{床y}}{{ラベル}}          … 台車(車輪が床に接する)
- ★禁止: 床に物体を置くのに生の \\ballobj{{x}}{{中心y}}{{r}} を使い、中心yを手計算すること(0.04だけ浮く等の事故が頻発する)。床上の球は必ず \\ballon を使う。
- 斜面上の物体は斜面に密着させる(\\begin{{scope}}[shift={{(接点x,接点y)}},rotate=θ] の中で底辺 y=0 の箱を描く)。浮かせない・めり込ませない。
- 壁・天井に接する物体やばねも接点座標を完全に一致させる。ばね(coil spring)は壁の面と物体の面の間にちょうど渡す。
- 重なり禁止: 寸法線・補助線・ラベルが本体の線や他のラベルと重ならないようにする。

=== 本番のリアルさ(参考の完成版に近づける・推奨) ===
- 小問集合(第1問)と考察では、本番のように「空欄 ア・イ に入る式/数値の組合せ」を問い、選択肢を \\cn 行の表(ア列・イ列)で選ばせる形式を多用する。単純な6択ばかりにしない。
  例:
    空欄 \\mb{{ア}}・\\mb{{イ}} に入る式の組合せとして最も適当なものを、次の \\cn{{1}}〜\\cn{{6}} のうちから一つ選べ。\\mb{{1}}
...
=== ラベルと矢印の衝突回避(厳守) ===
- ラベルは矢印・線・物体と必ず離す。矢印のラベルは矢の「中ほどの脇」または「先端の少し先」に node[above=2pt]/[right=2pt] で置き、矢印線の真上に重ねない。
- 2本の矢印やラベルが近い場合は、一方を上(above)、他方を下(below)に振り分けて分離する。
- 角度記号(θ など)の弧と角のラベルは、線分から十分内側に小さく描き、線に重ねない。
- 寸法線(dim)のラベルは寸法線の中点の外側(below/left)に置く。
- どうしても近接する場合は、ラベルに \\,(細い空白)や改行を使い、最小フォント(font=\\footnotesize/\\small)で逃がす。
...
- 物体・台車・球は必ず面(床・斜面・レール)に接して描く(隙間も食い込みも禁止)。床は \\groundstrip、床上の球は \\ballon、箱は \\boxon、立体は \\block を使う(生の \\ballobj を床に直接置かない)。
...
=== 図の情報量(ヒントを与えすぎない・最重要・厳守) ===
図は「問題を成立させるのに必要最小限の情報」だけを描く。答えや解法のヒントになるものは絶対に図に描かない。
- ★図に描いてはいけない(ヒント・答えの先取り):
    ・力の矢印とその数式ラベル(例: 摩擦力 $\\mu mg$、重力 $mg$、$mg\\sin\\theta$、磁気力 $BIl$、張力 $T$ 等)。
    ・重力やベクトルの分解(分力)・補助の角度・補助線で、まさにその問題で問うている量。
    ・崩壊後・衝突後など「結果」の速度/運動量ベクトル(向きや大小が答えになる場合)。
    ・求める量そのもの(停止距離・最高点の高さ 等)を図に矢印や寸法で明示すること。
- ○図に描いてよい(与えられた条件のみ):
    ・物体・装置の配置と形状、面・壁・床・斜面、回路や光学系のレイアウト。
    ・問題文で「与えられている」量の記号のみ(例: 初速 $v$、傾角 $\\theta$、質量 $m$、長さ $L$、間隔 $d$、距離 $D$、磁束密度 $B$ の向き)。
    ・向きの基準(正方向)・座標軸・必要な寸法線(与えられた長さのみ)。
- 具体例:
    ・「摩擦で止まるまでの距離」を問う問題 → 物体と初速 $v$ の矢印だけ。摩擦力 $\\mu mg$ の矢印は描かない。
    ・斜面の問題 → 斜面・台車・傾角 $\\theta$ だけ。重力やその分力 $mg\\sin\\theta$ は描かない。
    ・放射性崩壊で粒子と核の運動量の向きを問う問題 → 崩壊前の静止した核だけ。崩壊後の2本の矢印は描かない。
    ・磁場中の導体棒 → 棒・磁場の向き($\\odot$/$\\times$)・速度 $v$・長さ $l$ だけ。起電力やローレンツ力の矢印は描かない。
- 迷ったら描かない。図は「状況を正しく把握させる」ためのものであり、「解かせない」ためのものではない。

=== 選択肢が「図の集合」のとき(グラフ・打点・波形等・厳守) ===
- 1つの巨大な tikzpicture に複数の選択肢を詰め込まない(ラベルと図がずれる・ページ途中で割れる原因)。
- 各選択肢は独立した小さい tikzpicture にし、tabular のセルに「\\cn{{n}}\\,(図)」の形で入れる(\\gframe と同じ並べ方)。
- グラフ選択は \\gframe を使う。記録テープの打点パターン選択は提供済みの \\dotrow を使う:
    \\begin{{center}}\\setlength{{\\tabcolsep}}{{4pt}}
    \\begin{{tabular}}{{cc}}
    \\cn{{1}}\\,\\dotrow{{0.3,1.6,2.9,4.2,5.5,6.6}}  & \\cn{{2}}\\,\\dotrow{{0.3,1.0,1.7,2.4,3.1,3.8}} \\\\[1.6em]
    \\cn{{3}}\\,\\dotrow{{0.3,0.8,1.6,2.7,4.1,5.8}}  & \\cn{{4}}\\,\\dotrow{{0.3,2.2,3.8,5.0,5.8,6.3}} \\\\
    \\end{{tabular}}\\end{{center}}
  (等間隔=等速、間隔が右へ広がる=加速 など、パターンで意味を表す。)
- 図の選択肢セットは必ず1つの table/center にまとめ、各 \\cn ラベルを対応する図の直前に置く。図とラベルが別ページに分かれないようにする。
{custom_block}
=== 完遂ルール(最重要・絶対厳守) ===
- 模試1回分(問題・マークシート・自己採点欄・解答解説のすべて)を、省略せず最後まで完成させること。
- ★問題冊子(問題本文)だけで約28〜32ページの分量を必達。各大問に背景説明・会話文・図2〜3個・データ表を入れて厚くする(空白で埋めない)。
...
# 工学系: 図表・数式が多い分野は standard、暗記・知識系は lite
_SUBJECT_TIER_MAP = {
    # 暗記・知識中心
    '応用情報': 'lite', '情報': 'lite', 'プログラミング': 'lite',
    # 数式・図表中心
    '物理': 'standard', '数学': 'standard',
    '電気回路': 'standard', '電子回路': 'standard', '電磁気学': 'standard',
    '制御工学': 'standard', '信号処理': 'standard', '通信工学': 'standard',
    'ディジタル回路': 'standard', 'アルゴリズム': 'standard',
}
...
F4. 矢印: [->,thick] を使う（>=stealth は不要、preambleで自動設定済み）
F5. node内の数式は $...$ で囲む
...
F10. 力の分解: 元ベクトル=実線, 分解成分=dashed, 直角マーク=小正方形(0.2単位)
    \draw[dashed,->] (P)--(Px) node[below]{$F_x$};
    \draw ($(Px)+(0,0.2)$)--++(0.2,0)--++(0,-0.2); % 直角マーク
...
F13. 力ベクトルのラベルは \\draw とは別の \\node コマンドで矢印の中点に配置する。
     node[midway] をパスに載せると大きな矢印ヘッドで軸が見えなくなるため禁止。
     例: \\draw[->,blue,very thick] (2.2,3.35)--++(0,-1.0);
         \\node[right] at (2.2,2.85) {$mg$};
F14. モバイル表示対応（必須）:
     ・矢印は -{Stealth[length=4mm,width=3mm]} を使用（-> や >=stealth は禁止）
     ・線は thick 以上（thin は縮小で消える）
     ・circle の半径は 0.15 以上
     ・scale は 1.0〜1.2 の範囲で使用可
     例: \\draw[-{Stealth[length=4mm,width=3mm]},blue,very thick] (2.2,3.35)--++(0,-1.0);
         \\node[right] at (2.2,2.85) {$mg$};
...
\begin{tikzpicture}[>=stealth,scale=1.2]
  ...
  \begin{scope}[shift={(1.73,1.00)},rotate=30]
    ...
    \draw[->,red,very thick] (0,0.6)--++(0,1.0);
    \node[right] at (0,1.1) {$N$};
    % 摩擦力: scope内で斜面に沿って上向き
    \draw[->,green!60!black,thick] (-0.3,0.3)--++(-1.0,0);
    \node[left] at (-0.8,0.3) {$f$};
  \end{scope}
  % 重力: 世界座標で真下（物体中心の世界座標から）
  \draw[->,blue,very thick] (1.58,1.26)--++(0,-1.0);
  \node[right] at (1.58,0.76) {$mg$};
\end{tikzpicture}
...
\begin{tikzpicture}[>=stealth,scale=1.2]
  ...
  \begin{scope}[shift={(1.06,1.06)},rotate=45]
    ...
    \node at (0,0.25) {$m$};
    \draw[->,red,very thick] (0,0.5)--++(0,0.9);
    \node[right] at (0,0.95) {$N$};
  \end{scope}
  \draw[->,blue,very thick] (0.88,1.24)--++(0,-0.9);
  \node[right] at (0.88,0.79) {$mg$};
  % 運動方向（斜面下向き）
  \draw[->,orange,thick,dashed] (1.24,0.88)--++(0.35,-0.35);
  \node[right] at (1.42,0.71) {$a$};
\end{tikzpicture}
...
\begin{tikzpicture}[>=stealth,scale=1.0]
  ...
  \draw[->,blue,thick] (4.33,0.3)--++(0,-0.6);
  \node[right] at (4.33,0.0) {$m_B g$};
\end{tikzpicture}
...
\begin{tikzpicture}[>=stealth,scale=1.1]
  ...
  \node at (2.0,0.5) {$m$};
  \draw[->,red,very thick] (2.5,0.5)--++(1.2,0);
  \node[above] at (3.1,0.5) {$F$};
  \draw[->,blue,thick] (2.0,0.5)--++(0,-0.8);
  \node[right] at (2.0,0.1) {$mg$};
  \draw[->,green!60!black,thick] (2.0,1.0)--++(0,0.7);
  \node[right] at (2.0,1.35) {$N$};
\end{tikzpicture}
...
\begin{tikzpicture}[>=stealth,scale=1.1]
  ...
  \node at (2.55,0.35) {$m$};
  \draw[->,red,thick] (2.9,0.35)--++(0.9,0);
  \node[above] at (3.35,0.35) {$F$};
  \node at (1.17,0.62) {$k$};
\end{tikzpicture}
...
\begin{tikzpicture}[>=stealth,scale=1.1]
  ...
  \node at (0,1.45) {$m$};
  \draw[->,blue,thick] (0,1.25)--++(0,-0.85);
  \node[right] at (0,0.83) {$mg$};
  \draw[->,red,thick] (0,1.65)--++(0,0.7);
...
\begin{circuitikz}[scale=0.95,>=stealth]
  ...
  \draw (0,3) to[battery1,l_=$E$] (0,0)              % 左辺: 電池で下降
    ...
    -- (0,3);                                          % 上辺で始点に戻る
\end{circuitikz}
【電磁気: 直列＋並列 混合（電池+r+R_1 直列、R_2||R_3 並列） — ★Type-SP 典型例★】
% 状況: 電池 E（内部抵抗 r）に R_1 を直列、その先に R_2 と R_3 を並列接続。
% ★ R_2 と R_3 を矩形の隣接辺に置くと「直列」になってしまうため、独立した縦並列枝にする ★
\begin{circuitikz}[scale=1.0,>=stealth]
  % 上辺（直列部）: E+ → r → R_1 → 並列上母線右端
...
\end{circuitikz}
% NG例1: 単一矩形ループで R_2 を右辺、R_3 を下辺に → これは R_2-R_3 直列で意味が崩壊
% NG例2: I_2 矢印を独立 \draw[->] で描く → 素子からずれて表示
...
\begin{circuitikz}[scale=0.9,>=stealth]
  ...
  \draw (0,3) to[C,l_=$C_1$] (0,0)              % 左辺: C1 で下降
    -- (4,0)                                     % 下辺: 配線のみ
    to[C,l_=$C_2$] (4,3)                         % 右辺: C2 で上昇
    to[opening switch,l_=$S$] (0,3);             % 上辺: スイッチで左進、閉ループ
\end{circuitikz}

【電磁気: コンデンサ並列接続＋スイッチ（電池切り離し済み）— ★並列パターンの典型例★】
% 状況: C_1 が事前に充電され、電池を切り離した後、スイッチ S を閉じて C_2 と並列接続する。
% ★並列回路なので「一筆書き」は不可能★。母線2本＋並列枝を別々の \draw で描く。
\begin{circuitikz}[scale=1.0,>=stealth]
  % 上母線 A（スイッチ S を中央に挟む）
  \draw (0,2) -- (1.5,2) to[opening switch,l_=$S$] (2.5,2) -- (4,2);
  % 下母線 B
  \draw (0,0) -- (4,0);
  % 並列枝（縦・両端 x 一致）— ★絶対に斜めにしない★
  \draw (1,2) to[C,l=$C_1$] (1,0);
  \draw (3,2) to[C,l=$C_2$] (3,0);
  % ノードラベルは母線につき1回（右端 (4,2) には A を書かない）
  \node[left] at (0,2) {$A$};
  \node[left] at (0,0) {$B$};
\end{circuitikz}
% NG例: \node at (4,2) {$A$}; を追加する → 同じ電位を二重表示で混乱
% NG例: コンデンサの脚を斜めにする (1,2) to[C] (1.2,0) → 斜めは禁止
% NG例: 電池を消したのに (2,-0.3) node{$+$} (2.2,-0.3) node{$-$} を残す → 孤児禁止

【電磁気: 真の並列回路（枝が3本以上, circuitikz）— 外周一筆＋並列枝】
...
\begin{tikzpicture}[>=stealth,scale=1.0]
  ...
  \node at (4.5,-0.95) {$m_B$};
  % 力の矢印
  \draw[->,blue,thick] (4.5,-1.3)--++(0,-0.6);
  \node[right] at (4.5,-1.6) {$m_B g$};
  \draw[->,red,thick] (2.2,0.4)--++(0.8,0);
  \node[above] at (2.6,0.4) {$T$};
  \draw[->,blue,thick] (1.7,0)--++(0,-0.6);
  \node[right] at (1.7,-0.3) {$m_A g$};
\end{tikzpicture}
...
\begin{tikzpicture}[>=stealth,scale=1.0]
  ...
  \node at (2.8,0.35) {$m_2$};
  % 力の矢印
  \draw[->,blue,thick] (2.2,0.5)--++(0,-0.6);
  \node[right] at (2.2,0.2) {$m_1 g$};
  \draw[->,blue,thick] (2.8,0.0)--++(0,-0.6);
  \node[right] at (2.8,-0.3) {$m_2 g$};
\end{tikzpicture}
...
"""
    elif subject in ('電気回路', '電子回路', '電磁気学', '制御工学', '信号処理',
                     '通信工学', 'ディジタル回路'):
        figure_guide = r"""
【★★★ 電気回路 CircuiTikZ 図 — 必須ルール（厳守）★★★】
%%% FIGURE %%% の中に circuitikz で回路図を記述してください。
LaTeX をそのまま書けます（バックスラッシュの二重化は不要）。

■ コンパイルを確実にするルール:
F1. \begin{circuitikz}...\end{circuitikz} の対応を確認
F2. すべての文は ; で終わる
F3. \usepackage, \usetikzlibrary は書かない（preamble済み）

■ 描画方向ルール（最重要）:
D1. ★始点は必ず左上★、★反時計回り★に進む（左上→左下→右下→右上→左上）
D2. ★単一閉ループは1本の \draw で一筆書き★（辺ごとに分割しない）
D3. 素子は (始点) to[素子] (終点) の連続で書く。前後に余計な -- を入れない
D4. 電源は左辺に配置: \draw (0,H) to[battery1,l=$E$] (0,0) ... で +上 −下が標準向き
D5. invert フラグは使わない（必要になる時点で描画方向が間違っている）

■ 回路図パターン例（直流回路）:
\begin{circuitikz}[scale=0.9,>=stealth]
  \draw (0,2.8) to[battery1,l=$E$] (0,0)
        to[R,l=$R_1$] (4.5,0)
        to[R,l=$R_2$] (4.5,2.8)
        -- (0,2.8);
\end{circuitikz}
...
■ 単一閉ループ（コンデンサ＋スイッチ）:
\begin{circuitikz}[scale=0.9,>=stealth]
  \draw (0,3) to[C,l_=$C_1$] (0,0)
        -- (4,0)
        to[C,l_=$C_2$] (4,3)
        to[opening switch,l_=$S$] (0,3);
\end{circuitikz}

■ 並列回路（3本以上の枝）:
\begin{circuitikz}[scale=0.85,>=stealth]
  \draw (0,3) to[battery1,l=$E$] (0,0) -- (4.5,0) -- (4.5,3) -- (0,3);
  \draw (1.5,3) to[R,l=$R_1$] (1.5,0);
  \draw (3.0,3) to[R,l=$R_2$] (3.0,0);
\end{circuitikz}

図が不要な場合は %%% FIGURE %%% セクションを省略してください。
"""
    elif subject in ('応用情報', 'アルゴリズム', 'プログラミング'):
        figure_guide = r"""
【応用情報 図のガイドライン】
%%% FIGURE %%% の中に必要に応じて tikzpicture でフローチャート・E-R図・
ネットワーク図などを記述してください。コードは verbatim 環境で表示してください。

■ コンパイルを確実にするルール:
- \begin{tikzpicture}...\end{tikzpicture} の対応を確認
- すべての文は ; で終わる
- フローチャート: \tikzset{shape=rectangle,rounded corners} を活用
- verbatim 内ではバックスラッシュをそのまま書く
...
def generate_pdf(payload: dict = Body(...), background: BackgroundTasks = None):
    ...
    # Build a conservative LaTeX document preamble (safe defaults).
        ...
        def _comprehensive_latex_sanitize(tex: str) -> str:
            ...
            # 7f-3) ★ CircuiTikZ クリーンナップサニタイザ ★
            #     LLM がプロンプトに反して生成した典型バグを自動修正する。
            def _circuitikz_sanitize(tex_str):
                """LLM 出力の CircuiTikZ コード内によくある問題を自動修正する。
                対象:
                  (a) l=R_1 → l=$R_1$  （数式モード忘れ）
                  (b) to[battery1,...] -- (x,y) -- ... の直前に余計な to[short] が
                      連続している場合、その to[short] を削除（電池の脚二重化バグ防止）
                  (c) 全角の括弧・カンマ・数字を半角に変換
                  (d) 孤児の「+」「−」ノードテキスト（直近に電池/電圧源がない場合）の検出
                """
                pattern = r'(\\begin\{circuitikz\}(?:\[.*?\])?)([\s\S]*?)(\\end\{circuitikz\})'
                ...
                def _fix_env(m):
                    ...
                    # (a) l=R_1 や l=R12 等、数式モードを忘れた素子ラベルを修正
                    # 対象: to[X,l=Y_n] や to[X,l=Y] で Y が $...$ で囲まれていない場合
                    def _wrap_math(lm):
                        prefix = lm.group(1)  # 'l=' or 'l_=' or 'v=' or 'i=' など
                    ...
                    body = re.sub(
                        r'\b(l_?=|v=|i\^?>?\^?=)([A-Za-z][A-Za-z0-9_]*(?:_\{[^}]*\})?)\b',
                        _wrap_math, body
                    )

                    # (b) to[battery1,...] (a,b) to[short] (c,d) → 中間の to[short] を削除
                    #     電池の直後の不要な to[short] は電池脚の二重描画になる
                    body = re.sub(
                        r'(to\[(?:battery1|battery2|V|sV|I|sI)[^\]]*\])\s*\([^)]+\)\s*to\[short\]\s*',
                        r'\1 ',
                        body
                    )

                    # (c) 全角文字を半角に変換（座標などで誤入力されると致命的）
                    fullwidth_map = {
                        '（': '(', '）': ')', '，': ',', '；': ';',
                        '０': '0', '１': '1', '２': '2', '３': '3', '４': '4',
                        '５': '5', '６': '6', '７': '7', '８': '8', '９': '9',
                    ...
                    #     to[C], to[R] 等を使わずに矩形を描いている場合、その draw 行を除去。
                    body = re.sub(
                        r'\\draw\b[^;]*\brectangle\b[^;]*;\s*',
                        '', body
                    )

                    # (f) ★ コンデンサ素子の付近にある手動の +, -, − ノードを削除 ★
                    #     to[C] が含まれる circuitikz 環境では極性は v= オプションで描く。
                    #     手動で配置された ± テキストノードは削除する。
                    has_capacitor = bool(re.search(r'to\[(?:C|cC|eC|pC)\b', body))
                    if has_capacitor:
                        # \node[any] (at (x,y))? {$+$ or $-$ or − or +} を削除
                        body = re.sub(
                            r'\\node\b[^{;]*\{\s*\$?\s*[+\-−﹣－＋]\s*\$?\s*\}\s*;?\s*',
                            '', body
                        )

                    # (g) ★ コンデンサ素子の付近にある手動ラベル \node ... {$C_1$} 削除 ★
                    #     C_1, C_2, R_1, R_2 等のラベルは to[C,l=$C_1$] 等で組込済みのため。
                    #     ただし回路接続点ラベル (A, B, ノード名) は残す。
                    if has_capacitor:
                        body = re.sub(
                            r'\\node\b[^{;]*\{\s*\$([CR]_\{?\d+\}?|[CRL])\$\s*\}\s*;?\s*',
                            '', body
                        )
            ...
            try:
                tex = _circuitikz_sanitize(tex)
            except Exception as _e:
                logger.warning('circuitikz sanitize skipped: %s', _e)
...
def _preprocess_tikz_code(tikz_code: str) -> str:
    ...
    def _separate_node(m):
        ...
        delta_part = m.group(3).strip()
        node_opts = m.group(4).strip()
        node_text = m.group(5)

        # delta から数値を抽出: --++(dx,dy)
        delta_match = _re.match(r'--\+\+\(([^,]+),([^)]+)\)', delta_part)
        if not delta_match:
            return m.group(0)  # パースできなければ元のまま

        try:
            sx, sy = [float(x.strip()) for x in start_raw.split(',')]
            dx = float(delta_match.group(1).strip())
            dy = float(delta_match.group(2).strip())
            mx = round(sx + dx / 2, 2)
            my = round(sy + dy / 2, 2)
        except (ValueError, IndexError):
            # named coords 等: midway を除去して node[right] のみにする
            clean_opts = _re.sub(r'\bmidway\b,?\s*', '', node_opts).strip().rstrip(',')
            if not clean_opts:
                clean_opts = 'right'
            return f'{draw_part}({start_raw}){delta_part} node[{clean_opts}]{{{node_text}}};'

        # side を決定
        side = 'right'
        if 'left' in node_opts:
            side = 'left'
        elif 'above' in node_opts:
            side = 'above'
        elif 'below' in node_opts:
            side = 'below'

        return (
            f'{draw_part}({start_raw}){delta_part};\n'
            f'  \\node[{side}] at ({mx},{my}) {{{node_text}}};'
        )

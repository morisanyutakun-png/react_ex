  ...
  return (
    <div className="mb-6 sm:mb-8">
        ...
        <nav className="flex items-center gap-1.5 mb-4 sm:mb-5 text-xs font-semibold tracking-wide">
          {breadcrumbs.map((bc, i) => (
            <div key={i} className="flex items-center gap-1.5">
              {bc.href ? (
                <a href={bc.href} className="flex items-center gap-1 text-[#6e6e73] hover:text-red-600 transition-colors">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-4">
        {icon && (
          <div className="flex-shrink-0 w-10 h-10 rounded-xl bg-red-600 text-white flex items-center justify-center">
            {icon}
          </div>
        )}
        <div className="flex-1">
          ...
          {description && (
            <p className="text-sm text-[#6e6e73] mt-0.5 max-w-2xl leading-relaxed">
              {description}
            </p>
          )}
        </div>
    ...
    <div className={className}>
      ...
      <div className="relative group">
        <select
          ...
          className="w-full px-3 py-2.5 rounded-lg border border-black/[0.08] bg-white text-sm
  ...
  return (
    <div className={className}>
      ...
      <input
        ...
        max={max}
        step={step}
        className="w-24 px-3 py-2.5 rounded-lg border border-black/[0.08] bg-white text-sm text-[#1d1d1f]
  ...
  return (
    <div className={className}>
      ...
      <textarea
        ...
        readOnly={readOnly}
        className="w-full px-3.5 py-3 rounded-lg border border-black/[0.08] bg-white font-mono text-sm
  ...
  return (
    <div className={className}>
      ...
      <input
        ...
        onChange={(e) => onChange(e.target.value)}
        placeholder={placeholder}
        className="w-full px-3 py-2.5 rounded-lg border border-black/[0.08] bg-white text-sm text-[#1d1d1f]
                   transition-all hover:border-black/[0.12] focus:border-red-500 focus:ring-4 focus:ring-red-600/10
                   outline-none placeholder:text-[#aeaeb2]"
      />
    </div>
...
export function SectionCard({ title, subtitle, icon, children, className = '' }) {
  return (
    <div className={`bg-white rounded-xl border border-black/[0.06] p-5 sm:p-6 shadow-card ${className}`}>
      {(title || icon) && (
        <div className="mb-4">
          <div className="flex items-center gap-2">
            {icon && <span className="text-red-600">{icon}</span>}
            {title && <h2 className="text-sm sm:text-base font-semibold text-[#1d1d1f]">{title}</h2>}
          </div>
          {subtitle && <p className="text-xs text-[#6e6e73] mt-0.5 ml-6">{subtitle}</p>}
        </div>
      )}
      {children}
    </div>
...
export function EmptyState({ icon, title, description }) {
  return (
    <div className="text-center py-12 bg-[#f5f5f7] rounded-xl border border-dashed border-[#d2d2d7]">
      <div className="flex justify-center mb-3 text-[#d2d2d7]">
        {icon || <Icons.Empty />}
      </div>
      {title && <div className="text-sm font-semibold text-[#6e6e73] mb-0.5">{title}</div>}
      {description && <div className="text-xs text-[#aeaeb2]">{description}</div>}
    </div>
  );
}
...
export function Tabs({ tabs, activeTab, onTabChange }) {
  return (
    <div className="flex gap-0.5 p-0.5 bg-[#f5f5f7] rounded-xl">
      {tabs.map((tab) => (
        <button
          key={tab.id}

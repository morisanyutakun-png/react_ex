...
export default function TemplateSelector({
  ...
  return (
    <div>
        ...
        {showFieldInput && (
          <div>
            ...
            <input
              ...
              className="w-full pl-4 pr-4 py-3 rounded-2xl border border-black/10 bg-white text-[#1d1d1f] text-sm font-medium
                         transition-all duration-300 hover:border-[#ff375f]/40 hover:bg-white hover:shadow-md
                         focus:border-[#ff375f]/55 focus:ring-2 focus:ring-[#ff375f]/30/40 focus:shadow-md
                         outline-none placeholder:text-[#86868b] shadow-sm"
      ...
      {/* 選択中テンプレート情報 */}
      {selectedTemplate && (
        <div className="p-3 bg-[#fff5f7] rounded-xl mb-4 border border-black/10 ">
          <div className="text-sm font-semibold text-[#1d1d1f]">
            {selectedTemplate.name || selectedTemplate.id}
          </div>
          {selectedTemplate.description && (
            <div className="text-xs text-[#515154] mt-0.5">{selectedTemplate.description}</div>
          )}
      ...
      {/* 新規テンプレートフォーム */}
      {showNewForm && (
        <div className="mt-4 p-6 border border-[#ff375f]/40 rounded-xl bg-[#fff5f7] backdrop-blur-sm ">
          <div className="text-[15px] font-bold text-[#1d1d1f] mb-4 tracking-tight">新しいテンプレート</div>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 items-end">
            <div>
              <label className="block text-[11px] font-bold text-[#515154] uppercase tracking-wider mb-1.5">教科 *</label>
              <select
                value={newSubject}
                onChange={(e) => setNewSubject(e.target.value)}
                className="w-full px-3 py-2.5 rounded-xl border border-black/10 bg-white text-[#1d1d1f] text-sm input-ring focus:border-[#ff375f]/55 focus:ring-2 focus:ring-[#ff375f]/30/40"
              >
            ...
            {newSubject === '__custom' && (
              <div>
                <label className="block text-[11px] font-bold text-[#515154] uppercase tracking-wider mb-1.5">教科名</label>
                <input
            ...
            <div>
              <label className="block text-[11px] font-bold text-[#515154] uppercase tracking-wider mb-1.5">分野</label>
              <input
          ...
          <p className="mt-2 text-[#515154] text-[11px]">

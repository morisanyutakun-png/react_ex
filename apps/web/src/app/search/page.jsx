    ...
    <button onClick={onClick} type="button"
      className={`relative flex items-center gap-2 px-4 py-2.5 rounded-2xl text-[13px] font-bold
        ...
        ${selected
          ...
          : 'bg-[#ffffff] text-[#515154] border-black/10 hover:border-[#ff2d55]/50 hover:shadow-sm'
        } active:scale-[0.96]`}
      style={selected ? {
...
function DifficultySegment({ difficulties, value, onChange }) {
  return (
    <div className="flex gap-1 p-1 bg-[#fff5f7]/50 rounded-2xl border border-black/8">
      <button onClick={() => onChange('')} type="button"
        className={`px-3.5 py-2 rounded-[13px] text-[12px] font-bold transition-all duration-300
          ${!value ? 'bg-[#1e2d4a] text-[#1d1d1f] shadow-md' : 'text-[#86868b] hover:text-[#c4ccdd] hover:bg-[#1e2d4a]/50'}`}>
        全て
      </button>
          ...
          <button key={d.value} onClick={() => onChange(isActive ? '' : d.value)} type="button"
            className={`group relative px-3 py-2 rounded-[13px] text-[12px] font-bold transition-all duration-300
              ${isActive
                ...
                : 'text-[#86868b] hover:text-[#c4ccdd] hover:bg-[#1e2d4a]/50'
              }`}
            style={isActive ? {
    ...
    <div className="animate-in fade-in slide-in-from-top-2 duration-300">
      <div className="relative group">
        <select value={value} onChange={(e) => onChange(e.target.value)}
          className="w-full pl-4 pr-10 py-3 rounded-2xl border border-black/10 bg-[#ffffff] text-[13px]
                    text-[#1d1d1f] transition-all duration-300 cursor-pointer appearance-none font-semibold shadow-sm
                    hover:border-[#ff375f]/40 hover:bg-[#1a3224] hover:shadow-md
        ...
        <div className="absolute inset-y-0 right-3 flex items-center pointer-events-none text-[#86868b] group-hover:text-[#515154] transition-colors">
...
function Skeleton({ className = '' }) {
  return <div className={`animate-pulse bg-[#fff5f7] rounded-lg ${className}`} />;
}

function DetailBlock({ label, color = 'slate', children }) {
  const bgMap = { slate: 'bg-[#fff5f7]', blue: 'bg-[#ffe4ea]/50', indigo: 'bg-[#ffe4ea]/60', emerald: 'bg-[#ffe4ea]/60' };
  return (
    <div>
      <div className="text-[10px] font-bold text-[#86868b] mb-1.5">{label}</div>
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-4 sm:space-y-5 px-4 sm:px-5 py-6 sm:py-10 pb-8 sm:pb-12">
      ...
      <div className="text-center mb-2 relative">
        ...
        <h1 className="text-[22px] sm:text-[26px] font-bold text-[#1d1d1f] tracking-[-0.02em] mb-1.5">問題をさがす</h1>
        <p className="text-[13px] text-[#86868b] leading-relaxed max-w-md mx-auto">
      ...
      {/* ── 検索パネル ── */}
      <div className="relative overflow-hidden rounded-[20px] bg-[#ffffff] border border-black/10"
        ...
        <div className="p-4 sm:p-7 space-y-4 sm:space-y-5">
          ...
          {/* ── 検索バー ── */}
          <div className="relative group">
            <div className="relative flex items-center gap-2 bg-[#fff5f7]/40 rounded-[20px] border border-black/10
                            group-focus-within:border-[#ff375f]/60 group-focus-within:shadow-md
                            transition-all duration-300">
              <div className="flex items-center justify-center w-10 h-10 sm:w-11 sm:h-11 ml-1.5 rounded-2xl bg-[#fff5f7] flex-shrink-0">
                <Icons.Search className="w-[18px] h-[18px] text-[#515154]" />
              </div>
              <input
                ref={inputRef}
                type="text"
                ...
                className="flex-1 min-w-0 py-3.5 pr-2 bg-transparent text-[15px] font-medium text-[#1d1d1f] outline-none placeholder:text-[#86868b]"
          ...
          {/* ── 科目チップ ── */}
          <div>
            <div className="text-[10px] font-bold text-[#86868b] uppercase tracking-[0.15em] mb-2.5 px-0.5">科目</div>
          ...
          {/* ── 難易度 ── */}
          <div>
            <div className="text-[10px] font-bold text-[#86868b] uppercase tracking-[0.15em] mb-2.5 px-0.5">難易度</div>
          ...
          {hasActiveFilters && (
            <div className="flex items-center gap-2 pt-3 border-t border-black/8">
              <span className="text-[10px] font-bold text-[#86868b] flex-shrink-0">絞り込み</span>
              ...
              <button onClick={clearFilters}
                className="text-[11px] text-[#86868b] hover:text-[#1d1d1f] transition-all font-semibold
                           flex items-center gap-1 px-3 py-1.5 rounded-xl hover:bg-[#fff5f7]/50 flex-shrink-0">
      ...
      {!searching && results.length > 0 && (
        <div className="space-y-3">
          <div className="text-[11px] text-[#86868b] font-bold uppercase tracking-wider px-1">{totalCount} 件の結果</div>
              ...
              <div key={item.id ?? idx}
                ...
                <div className="p-5">
                  <div className="flex items-start gap-3">
                    <span className="text-[10px] text-[#86868b] font-mono mt-0.5 flex-shrink-0 w-8 text-right">#{item.id ?? idx + 1}</span>
                    <div className="flex-1 min-w-0">
                      <div className="text-[13px] text-[#1d1d1f] leading-relaxed">
                        <LatexText>{(item.stem || item.text || '').slice(0, 200)}</LatexText>
                        {(item.stem || item.text || '').length > 200 ? <span className="text-[#86868b]">...</span> : ''}
                      </div>
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      {item.score != null && (
                        <span className="text-[10px] text-[#86868b] font-mono tabular-nums px-1.5 py-0.5 rounded-lg bg-[#fff5f7]/50">{Number(item.score).toFixed(3)}</span>
                      )}
                      <svg className={`w-4 h-4 text-[#86868b] transition-transform duration-300 ${isOpen ? 'rotate-180 text-[#1d1d1f]' : ''}`}
                ...
                {/* ── 展開コンテンツ ── */}
                {isOpen && (
                  <div className="px-5 pb-5 space-y-3 border-t border-black/10 animate-expand" onClick={(e) => e.stopPropagation()}>
                    <div className="pt-4" />
                    ...
                    <div className="flex items-center gap-3 pt-2">
                      ...
                      <button onClick={() => { navigator.clipboard.writeText(item.stem || item.text || ''); setStatus('問題文をコピーしました'); }}
                        className="inline-flex items-center gap-1.5 px-3.5 py-2.5 rounded-2xl text-xs font-bold
                                   text-[#515154] bg-[#ffffff] border border-black/10
                    ...
                    {similarResults[item.id] && similarResults[item.id].length > 0 && (
                      <div className="mt-3 space-y-2">
                        <div className="text-xs font-semibold text-[#1d1d1f] uppercase">
                          生成された類題 ({similarResults[item.id].length}件)
                        </div>
                        {similarResults[item.id].map((sim, sIdx) => (
                          <div key={sIdx} className="bg-[#ffe4ea]/50 rounded-lg p-3 border border-black/10">

...
function Badge({ children, color = 'slate' }) {
  const map = {
    ...
    slate: 'bg-blue-50/60 text-[#7ab896] border-blue-200/50',
    ...
    <button onClick={onClick} type="button"
      className={`relative flex items-center gap-2 px-4 py-2.5 rounded-2xl text-[13px] font-bold
        ...
        ${selected
          ...
          : 'bg-[#122a1c] text-[#a8d8bb] border-emerald-800/40 hover:border-emerald-600/50 hover:shadow-sm'
        } active:scale-[0.96]`}
      style={selected ? {
    ...
    <div className="flex gap-1 p-1 bg-blue-50/50 rounded-2xl border border-blue-200/40">
      <button onClick={() => onChange('')} type="button"
        className={`px-3.5 py-2 rounded-[13px] text-[12px] font-bold transition-all duration-300
          ${!value ? 'bg-[#1e2d4a] text-[#e8f5ed] shadow-md' : 'text-[#9dc8b0] hover:text-[#c4ccdd] hover:bg-[#1e2d4a]/50'}`}>
        全て
      </button>
          ...
          <button key={d.value} onClick={() => onChange(isActive ? '' : d.value)} type="button"
            className={`group relative px-3 py-2 rounded-[13px] text-[12px] font-bold transition-all duration-300
              ${isActive
                ...
                : 'text-[#9dc8b0] hover:text-[#c4ccdd] hover:bg-[#1e2d4a]/50'
              }`}
            style={isActive ? {
    ...
    <div className="animate-in fade-in slide-in-from-top-2 duration-300">
      <div className="relative group">
        <select value={value} onChange={(e) => onChange(e.target.value)}
          className="w-full pl-4 pr-10 py-3 rounded-2xl border border-emerald-800/40 bg-[#122a1c] text-[13px]
                    text-[#e8f5ed] transition-all duration-300 cursor-pointer appearance-none font-semibold shadow-sm
                    hover:border-blue-300/50 hover:bg-[#1a3224] hover:shadow-md
        ...
        <div className="absolute inset-y-0 right-3 flex items-center pointer-events-none text-[#c7c7cc] group-hover:text-[#7ab896] transition-colors">
...
function DetailBlock({ label, color = 'slate', children }) {
  ...
  return (
    <div>
      ...
      <div className={`text-sm text-[#e8f5ed] ${bgMap[color] || bgMap.slate} rounded-lg p-3 leading-relaxed`}>
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-4 sm:space-y-5 px-4 sm:px-5 py-6 sm:py-10 pb-8 sm:pb-12">
      ...
      <div className="text-center mb-2 relative">
        ...
        <h1 className="text-[22px] sm:text-[26px] font-bold text-[#e8f5ed] tracking-[-0.02em] mb-1.5">問題をさがす</h1>
        <p className="text-[13px] text-[#9dc8b0] leading-relaxed max-w-md mx-auto">
      ...
      {/* ── 検索パネル ── */}
      <div className="relative overflow-hidden rounded-[20px] bg-[#122a1c] border border-emerald-800/40"
        ...
        <div className="p-4 sm:p-7 space-y-4 sm:space-y-5">
          ...
          <div className="relative group">
            <div className="relative flex items-center gap-2 bg-blue-50/40 rounded-[20px] border border-blue-200/50
              ...
              <div className="flex items-center justify-center w-10 h-10 sm:w-11 sm:h-11 ml-1.5 rounded-2xl bg-blue-50/60 flex-shrink-0">
                <Icons.Search className="w-[18px] h-[18px] text-[#7ab896]" />
              </div>
              <input
                ref={inputRef}
                type="text"
                ...
                className="flex-1 min-w-0 py-3.5 pr-2 bg-transparent text-[15px] font-medium text-[#e8f5ed] outline-none placeholder:text-[#c7c7cc]"
          ...
          {/* ── 科目チップ ── */}
          <div>
            <div className="text-[10px] font-bold text-[#9dc8b0] uppercase tracking-[0.15em] mb-2.5 px-0.5">科目</div>
          ...
          {/* ── 難易度 ── */}
          <div>
            <div className="text-[10px] font-bold text-[#9dc8b0] uppercase tracking-[0.15em] mb-2.5 px-0.5">難易度</div>
          ...
          {hasActiveFilters && (
            <div className="flex items-center gap-2 pt-3 border-t border-blue-200/40">
              ...
              <div className="flex flex-wrap gap-1.5 flex-1 min-w-0">
                ...
                {fieldFilter && (
                  <button onClick={() => setFieldFilter('')}
                    className="inline-flex items-center gap-1.5 text-[11px] font-semibold px-3 py-1 rounded-full
                               bg-blue-100/60 text-[#e8f5ed] border border-blue-300/50 hover:bg-[#2563eb]/[0.14] transition-colors">
                    {fieldFilter}
                    <svg className="w-3 h-3 opacity-50" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
                  </button>
                )}
                {difficultyFilter && (
                  <button onClick={() => setDifficultyFilter('')}
                    className="inline-flex items-center gap-1.5 text-[11px] font-semibold px-3 py-1 rounded-full
                               bg-blue-100/50 text-[#6aaa7c] border border-blue-300/50/[0.12] hover:bg-[#475569]/[0.14] transition-colors">
                    {difficultyFilter}
                    <svg className="w-3 h-3 opacity-50" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
                  </button>
                )}
              ...
              <button onClick={clearFilters}
                className="text-[11px] text-[#9dc8b0] hover:text-[#e8f5ed] transition-all font-semibold
      ...
      {!searching && results.length > 0 && (
        <div className="space-y-3">
          <div className="text-[11px] text-[#9dc8b0] font-bold uppercase tracking-wider px-1">{totalCount} 件の結果</div>
            ...
            return (
              <div key={item.id ?? idx}
                className={`result-item transition-all duration-300 
                  ${isOpen ? '!border-[#2563eb]/30 ring-1 ring-[#2563eb]/10 !bg-[#122a1c] shadow-lg' : ''}`}
                onClick={() => setExpandedId(isOpen ? null : (item.id ?? idx))}>
                <div className="p-5">
                  <div className="flex items-start gap-3">
                    <span className="text-[10px] text-[#9dc8b0] font-mono mt-0.5 flex-shrink-0 w-8 text-right">#{item.id ?? idx + 1}</span>
                    <div className="flex-1 min-w-0">
                      <div className="text-[13px] text-[#e8f5ed] leading-relaxed">
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      {item.score != null && (
                        <span className="text-[10px] text-[#9dc8b0] font-mono tabular-nums px-1.5 py-0.5 rounded-lg bg-blue-50/50">{Number(item.score).toFixed(3)}</span>
                      )}
                      <svg className={`w-4 h-4 text-[#c7c7cc] transition-transform duration-300 ${isOpen ? 'rotate-180 text-[#e8f5ed]' : ''}`}
                ...
                {/* ── 展開コンテンツ ── */}
                {isOpen && (
                  <div className="px-5 pb-5 space-y-3 border-t border-emerald-800/40 animate-expand" onClick={(e) => e.stopPropagation()}>
                    <div className="pt-4" />
                    ...
                    <div className="flex items-center gap-3 pt-2">
                      ...
                      <button onClick={() => { navigator.clipboard.writeText(item.stem || item.text || ''); setStatus('問題文をコピーしました'); }}
                        className="inline-flex items-center gap-1.5 px-3.5 py-2.5 rounded-2xl text-xs font-bold
                                   text-[#a8d8bb] bg-[#122a1c] border border-emerald-800/40
                                   hover:bg-[#1a3224] hover:shadow-md hover:-translate-y-0.5 transition-all active:scale-95">
                        <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    {similarResults[item.id] && similarResults[item.id].length > 0 && (
                      <div className="mt-3 space-y-2">
                        <div className="text-xs font-semibold text-[#e8f5ed] uppercase">
                        ...
                        {similarResults[item.id].map((sim, sIdx) => (
                          <div key={sIdx} className="bg-blue-100/50 rounded-lg p-3 border border-blue-200/50">
                            <LatexBlock className="text-xs text-[#e8f5ed]">
      ...
      {!searching && !hasSearched && (
        <div className="text-center py-12 sm:py-16">
          ...
          <h3 className="text-[17px] font-bold text-[#e8f5ed] mb-2">キーワードや科目で検索</h3>
          <p className="text-[13px] text-[#9dc8b0] max-w-sm mx-auto leading-relaxed mb-6">
          ...
          <div className="flex flex-wrap justify-center gap-2 max-w-xs mx-auto">
            {['二次関数', '微分', '力学', '英文法'].map((hint) => (
              <button key={hint} onClick={() => { setQuery(hint); }}
                className="px-3 py-1.5 rounded-full text-[12px] font-semibold text-[#a8d8bb] bg-[#122a1c] border border-emerald-800/40 
